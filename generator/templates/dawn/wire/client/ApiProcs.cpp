...
namespace {{wire_namespace}}::client {
    ...
    {% set prefix = metadata.c_prefix.lower() %}
    namespace {
        {{PREFIX}}Instance ClientCreateInstance({{PREFIX}}InstanceDescriptor const* descriptor) {
            UNREACHABLE();
            return nullptr;
        }
        ...
        static const ProcEntry sProcMap[] = {
            {% for (type, method) in c_methods_sorted_by_name %}
                { reinterpret_cast<{{PREFIX}}Proc>(Client{{as_MethodSuffix(type.name, method.name)}}), "{{as_cMethod(type.name, method.name)}}" },
            {% endfor %}
        };
        static constexpr size_t sProcMapSize = sizeof(sProcMap) / sizeof(sProcMap[0]);
    }  // anonymous namespace

    //* {{PREFIX}}Proc ClientGetProcAddress({{PREFIX}}Device, const char* procName) {
    {{PREFIX}}Proc ClientGetProcAddress({{PREFIX}}Context, const char* procName) {
        if (procName == nullptr) {
            return nullptr;
        }
        ...
        if (strcmp(procName, "{{prefix}}GetProcAddress") == 0) {
            return reinterpret_cast<{{PREFIX}}Proc>(ClientGetProcAddress);
        }

        if (strcmp(procName, "{{prefix}}CreateInstance") == 0) {
            return reinterpret_cast<{{PREFIX}}Proc>(ClientCreateInstance);
        }
